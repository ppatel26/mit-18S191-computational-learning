...
unused_letters = let
	alphabet[findall(sample_freqs .== 0)]
end
...
double_letters = alphabet[map(i -> i[2], findall(Diagonal(sample_freq_matrix) .!= 0))]
...
# ╔═╡ cd3272e0-fc69-11ea-3d9f-c1a731f0a5c5
md"**Sum of each Column =** $(sum(sample_freq_matrix,dims=1))"

# ╔═╡ d36fd08a-fc69-11ea-1e58-5d2c2404e398
md"**Sum of each Rows =** $(sum(sample_freq_matrix,dims=2))"

# ╔═╡ cc62929e-f9af-11ea-06b9-439ac08dcb52
row_col_answer = md"""
sums of each rows and columns tell us if the particular alphabet is more likely used as a "preceding" letter or "following" letter respectively.
"""
...
# ╔═╡ f4a972de-fc77-11ea-0521-ab554fc26e87
md"**Rough Work**"

# ╔═╡ fc2e574a-fc77-11ea-358a-110eff34e078
length(forest_words)

# ╔═╡ 06d3ccc0-fc78-11ea-121e-e134750f2400
length(emma_words)
...
function ngrams(words, n)
	return [words[i:i+n-1] for i=1:(length(words) - n)+1]
end
...
function word_counts(words::Vector)
	...
	for word in words
		if !haskey(counts, word)
			counts[word] = length(findall(words.==word))
		end
	end
...
function completions_cache(grams)
	...
	for gram in grams
		cache[gram[1:n-1]] = vcat(gram[end], get!(cache, gram[1:n-1], gram[end]))
	end
